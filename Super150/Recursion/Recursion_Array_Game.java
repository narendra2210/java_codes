...
public class Recursion_Array_Game {
    public static void main(String args[]) {
        ...
        while(t>0){
            ...
            System.out.println(Array_game(arr,0,n-1));
        }
    }
    public static int sum_of_array(int arr[],int start,int end){
        int sum=0;
    ...
    public static int Array_game(int arr[],int start,int end){
        ...
        if(sum_of_array(arr,0,arr.length-1)==0){
            return arr.length-1;
        }
        for(int i=0;i<size;i++){
            if(sum_of_array(arr,start,start+i)==sum_of_array(arr,start+i+1,end)){
                count = count+1;
                return count + Math.max(Array_game(arr,start,start+i) , Array_game(arr,start+i+1,end));
            }
        }
        return count;
    }
